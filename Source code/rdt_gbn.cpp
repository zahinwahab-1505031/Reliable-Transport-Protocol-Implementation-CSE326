...
{
  //  fprintf(fp,"buffer er size: %d",(int)pkt_buffer.size());
    fprintf(fp, "FROM UPPER LAYER A GOT A MESSAGE: %s\n",message.data);
    ...
    if(matchCheckSum(packet)==1) {
        if(fallsWithinWindow(packet.acknum)) {
            ...
            while(!pkt_buffer.empty()) {
              struct  pkt popped = pkt_buffer.front();
                if(fallsWithinWindow(popped.seqnum)){
                   fprintf(fp,"====A IS SENDING FOLLOWING PACKET FROM BUFFER==================\n");
                  
                    pkt_buffer.pop();
                      printPacket(popped);
                    senderWindow[popped.seqnum] = packet;
                    tolayer3(A,popped);
                    myStartTimer(A,INCREMENT);
                }
            }
